...
- **完成后调用 `checkpoint({ "checkpoint_id": "take_screenshot_target", "iteration_number": N, "evidence": "已确认target_image.png存在" })`**
...
- **完成后调用 `checkpoint({ "checkpoint_id": "read_screenshots", "iteration_number": N, "evidence": "已读取target_image.png和复刻页面截图" })`**
...
- **完成后调用 `checkpoint({ "checkpoint_id": "find_differences", "iteration_number": N, "evidence": "找出的差异点列表（至少5处，全部从截图视觉对比中获取）" })`**
...
- **修复完所有差异点后，调用 `checkpoint({ "checkpoint_id": "fix_differences", "iteration_number": N, "evidence": "修改的文件列表和修复的差异点" })`**
...
1. **识别缺失的图片资源**
   - 从 `target_image.png` 中识别缺失的图片，或从原网站提取图片 URL（使用 `browser_evaluate` 查询元素的 `background-image` 或 `<img>` 标签的 `src` 属性）
   - **🚨 重要：必须去原网页确认元素类型**：
     - **必须搞清楚原网页的对应元素到底是文字还是图片**
     - 使用 `browser_navigate` 打开原网站
     - 使用 `browser_evaluate` 检查元素类型：
       - 如果是 `<img>` 标签或 `background-image`，则是图片
       - 如果是文字节点，则是文字
     - **如果是图片，直接下载图片，不要用文字替代**
   ...
   - 检查复刻项目中是否已有该图片
   - **🚨 如果缺失的是图片，必须从网页下载**：
     - 使用 `browser_navigate` 打开原网站（如果需要）
     - 使用 `browser_evaluate` 获取图片 URL（查询元素的 `background-image` 或 `<img>` 标签的 `src` 属性）
     - 使用 `fetch` API 或可用的 MCP 工具下载图片
