...
**你的核心任务（必须完整执行，不是计划）**：
1. ✅ 利用 MCP 工具获取需求
2. ✅ 为每个需求编写测试用例（RED 阶段）
3. ✅ **立即实现完整功能代码**（GREEN 阶段）- 包括完整的 UI、样式、交互、API、业务逻辑
4. ✅ **运行测试直到全部通过**
5. ✅ **验证页面效果**（启动服务，确认视觉和交互完整）
6. ✅ 循环处理所有需求，直到全部完成
...
3. **视觉验证**（如果涉及 UI）：
   ...
   - 确认布局、颜色、字体、图片都正确显示
   - 确认交互行为符合预期
4. **输出完成状态**：明确告知用户"需求 {id} 已完成，功能完整可用"
5. **立即返回 Step 1**，调用 `pop_req_to_implement` 获取下一个需求
6. **重复循环**，直到工具返回 "All requirements completed"
...
3. **视觉还原验证（强制执行）**：
   ...
   - 转换图片路径为绝对路径：`项目根目录/requirements/images/文件名.扩展名`
...
5. **参考图片访问规范**：
   - 参考图片位于 `requirements/images/` 目录
   - 图片路径在需求的 `description` 字段中声明（相对路径）
   - 转换为绝对路径：`项目根目录/requirements/images/文件名.扩展名`
...
### 完成确认
- ✅ 已在响应中明确告知用户"需求 X 已完成，功能完整可用"

**⚠️ 只有完成以上所有步骤，且在浏览器中确认页面效果完整，才能继续下一个需求。**
...
**🎯 一句话标准**：用户启动服务后，应该能在浏览器中看到一个**完整的、可交互的、视觉还原设计稿的登录页面**，并且可以成功完成登录流程，同时你应该准备好测试帐号帮助用户快速判断功能是否正常实现。