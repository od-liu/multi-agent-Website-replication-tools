...
import { authenticateUser, sendVerificationCode, verifySmsCode } from '../../src/database/operations.js';
...
describe('FUNC-VERIFY-SMS-CODE', () => {
  ...
  it('should return error for expired code', async () => {
    ...
    // For now, we'll manually insert an expired code
    const db = new sqlite3.Database(TEST_DB_PATH);
    ...
    await new Promise((resolve) => {
      db.get('SELECT id FROM users WHERE username = ?', ['testuser'], (err, user) => {
        ...
        db.run(
          'INSERT INTO verification_codes (user_id, code, type, expires_at, used) VALUES (?, ?, ?, ?, ?)',
          [user.id, '123456', 'sms', expiredTime, 0],
          () => {
            db.close();
            resolve();
          }
    ...
    const result = await verifySmsCode('testuser', '1234', '123456');
